...
LifeLed ::LifeLed(const char* const compName) : LifeLedComponentBase(compName), m_blink_count(0), m_pin(Atmel::DEF_LED_BUILTIN), m_configured(false) {}
...
void LifeLed::configure(PlatformIntType pin) {
    ...
    digitalWrite(this->m_pin, Atmel::DEF_LOW);
    this->m_configured = true;
}

void LifeLed::set(const Fw::On::T &on_off) {
    if (this->m_configured) {
        digitalWrite(this->m_pin, (on_off == Fw::On::ON) ? Atmel::DEF_HIGH : Atmel::DEF_LOW);
    }
}
